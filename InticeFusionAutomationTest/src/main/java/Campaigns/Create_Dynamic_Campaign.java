...
public class Create_Dynamic_Campaign extends Page
{
	...
	{
		driver.findElement(By.xpath(OR.getProperty("dynamicdisplay"))).click();
		System.out.println("selected dynamic campaign");
		log.debug("selected dynamic campaign");
...
//			driver.findElement(By.name(OR.getProperty("creativeids"))).sendKeys(creativeids);
			...
			String campaignname, String campaignname1) throws InterruptedException
	{
			...
			driver.findElement(By.name(OR.getProperty("campaigncontent"))).sendKeys(campaigncontent);
			System.out.println("entered campaign content with spaces");
			log.debug("entered campaign content with spaces");
			Thread.sleep(1000);
			String invalidContent = driver.findElement(By.xpath(OR.getProperty("invalidcontent"))).getText();
			System.out.println(invalidContent + " validation displayed");
			log.debug(invalidContent +" validation displayed");
			if(driver.findElement(By.xpath(OR.getProperty("invalidcontent"))).isDisplayed())
			{
				System.out.println("Validation is working fine for Campaign Content if it has spaces");
				log.debug("Validation is working fine for Campaign Content if it has spaces");
				driver.findElement(By.name(OR.getProperty("campaigncontent"))).clear();
				driver.findElement(By.name(OR.getProperty("campaigncontent"))).sendKeys(campaigncontent1);
				System.out.println("entered campaign content without spaces");
				log.debug("entered campaign content without spaces");
			}
			else
			{
				System.out.println("Validation is not working for campaign content if it has spaces");
				log.debug("Validation is not working for campaign content if it has spaces");
			}
					
//			Actions act = new Actions(driver);
//									
//			WebElement content= driver.findElement(By.id(OR.getProperty("campaignContent")));
//			act.contextClick(content).build().perform();
//			driver.findElement(By.linkText(OR.getProperty("key1"))).click();
//					
//			act.contextClick(content).build().perform();
//			driver.findElement(By.linkText(OR.getProperty("key2"))).click();
//				
//			act.contextClick(content).build().perform();
//			driver.findElement(By.linkText(OR.getProperty("key3"))).click();
	...
	public void campaign_Impression_Details(String impressionperrecord, String sheetName)
	{
		int rows = list.getRowCount(sheetName);
	...
	{
		topNav.nexter();
		System.out.println("Click on next without selecting template");
		log.debug("Click on next without selecting template");
		String template_validation = driver.findElement(By.xpath(OR.getProperty("templatealert"))).getText();
		System.out.println(template_validation + " validation displayed");
		log.debug(template_validation + " validation displayed");
		
		if(driver.findElement(By.xpath(OR.getProperty("templatealert"))).isDisplayed())
		{
			System.out.println("Validation is working correctly if click on next without selecting template");
			log.debug("Validation is working correctly if click on next without selecting template");;
			driver.findElement(By.linkText(OR.getProperty("select"))).click();
			System.out.println("Template selected");
			log.debug("Templated selected");
		}
		System.out.println("Navigate to Customized template");
		log.debug("Navigate to Customized template");
	}
